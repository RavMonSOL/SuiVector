...
export default function MobileNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-muted flex items-center justify-around py-2 px-4 z-30">
      <Link href="/" className={`flex flex-col items-center p-2 ${location === '/' ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-home"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        <span className="text-xs mt-1">Home</span>
      </Link>
      
      <Link href="/marketplace" className={`flex flex-col items-center p-2 ${location === '/marketplace' ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}>
      ...
      <Link href="/explore" className={`flex flex-col items-center p-2 ${location === '/explore' ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}>
      ...
      <Link href="/wallet" className={`flex flex-col items-center p-2 ${location === '/wallet' ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}>
      ...
      <Link href="/profile" className={`flex flex-col items-center p-2 ${location === '/profile' ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}>
